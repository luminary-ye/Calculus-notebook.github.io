...
> [!tip] 类型检查
> 先看输入输出类型，许多错误会自动暴露：散度的输出是标量，旋度的输出是向量，梯度的输出也是向量。
...
第一条说，实际来自势函数的梯度场没有局部旋转。第二条说，任何旋度场都没有局部净源汇。
...
> [!success] 阶段小结
> $\nabla$ 的组合规则把向量微积分的计算组织起来：梯度场无旋，旋度场无源。实际做题时还要记住区域条件，不能把局部恒等式误用成无条件的全局结论。
