...
> [!tip] 怎么想
> Stokes 定理是二维曲面上的“望远镜效应”：许多小块的内边界一正一反互相抵消，最后只留下最外层边界。公式右边看起来在算曲面内部，左边看起来只绕边界走一圈，但它们记录的是同一批局部旋转的总效果。
...
> [!info] 符号说明
> $\nabla\times\vec F$ 是一个向量场；$(\nabla\times\vec F)\cdot\vec n$ 是一个标量函数。Stokes 定理右边其实是一个通量积分，只不过穿过曲面的向量场不是原来的 $\vec F$，而是它的旋度 $\nabla\times\vec F$。
...
Stokes 定理里最容易错的不是求导，而是方向。曲面方向和边界方向不能各选各的。
...
这个配套也解释了为什么 Stokes 定理不是单纯地说“曲面边界随便绕一圈”。它说的是：定向曲面诱导出定向边界，然后这两个定向对象的积分相等。
...
> \pi.
> $$
> 不必知道 $S$ 在内部怎样弯曲。真正决定结果的是同一条边界曲线和相容的方向。

这和保守场中的路径无关有一点相似：那里线积分只看端点；这里旋度通量只看边界曲线。不过不要把两者混成同一个定理。一个讨论的是梯度场的线积分，一个讨论的是旋度场的曲面通量。
...
3. 为什么换曲面时必须保证边界完全相同，而不是“看起来差不多围住同一片区域”？
