...
> [!abstract] 本节主线
> [[04-数学/00-微积分/07-微分方程/05-级数、边值问题与 PDE/02-Fourier、边值问题与 PDE 接口|Fourier、边值问题与 PDE 接口]] 已经说明“边界筛模式，初始形状定系数”。本页补上入门计算里最容易漏掉的细节：半区间正弦/余弦展开、一般区间长度、收敛读法、非零边界热方程、Laplace 方程和带初速度的弦振动。
...
零边界热方程可以直接用正弦模式。若端点温度不是零，先不要硬套正弦展开。
